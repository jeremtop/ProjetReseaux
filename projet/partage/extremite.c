...
void ext_out(char *port, int tunnel) {
	...
	if ((sock = socket(resol->ai_family, resol->ai_socktype, resol->ai_protocol)) <
      0) {
	...
	if(bind(sock, resol->ai_addr, sizeof(struct sockaddr_in6)) < 0) {
		perror("bind");
		exit(5);
	}
	...
	int resolaccept;
	if ((resolaccept=accept(sock, (struct sockaddr *)&client, (socklen_t *)&len)) < 0) {
		perror("accept");
		exit(7);
	}
...
int main(int argc, char** argv) {
	...
	if (f != 0){
		...
		printf("Le serveur est configuré.\n");
		getchar();
